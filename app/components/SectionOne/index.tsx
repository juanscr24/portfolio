        ...
        <div className='w-3/4 bg-gray-900 flex flex-col items-center px-16 py-10 gap-2 max-lg:w-full'>
            ...
            <div className='flex mt-5 flex-wrap justify-between'>
                {cardImage.map((option, index) => {
                    return (
                        <CardWork openImage={() => setShowModal(true)} key={index} title={option.title} desc={option.desc} img={option.img} />
                    )
                })}
            </div>
            <div className='flex justify-center'>
                <Modal isVisible={showModal} onClose={() => setShowModal(false)}>
                    {cardImage.map((option, index) => {
                        const validation = option.img === showId
                        return (
                            <Image src={option.img} key={index} width={500} height={500} alt='hola' />
                        )
                    })}
                </Modal>
            </div>
